...
| ----------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Configuration & Secrets | Secure, repeatable environment setup. | Supabase keys rotated, configuration injected at build, onboarding doc published.                                        | 1. Add `.env.example` with Supabase vars.<br>2. Add build step (Vite or similar) that consumes env vars and outputs `config.js`.<br>3. Document setup in `docs/setup.md`.<br>4. Script to rotate Supabase service/anon keys and update secrets store.<br>5. Enable Row Level Security, define roles & policies.<br>6. Add CI job that fails if secrets accidentally committed (secret scan). |
| Developer Experience    | Guardrails for consistency.           | ESLint + Prettier enforced, TypeScript/JSDoc adopted, unit/E2E testing baseline in place, GitHub Actions pipeline green. | 1. Introduce tooling (ESLint, Prettier) with configs.<br>2. Convert critical modules to TypeScript or add JSDoc types.<br>3. Add Vitest with sample tests (render games table, filter logic).<br>4. Add Playwright smoke test (modal open/close).<br>5. Configure GitHub Actions to run lint, tests, secret scan.<br>6. Update CONTRIBUTING with coding standards & review checklist.        |
| Data Pipeline & Backend | Reliable Supabase data model.         | Migration files committed, seeding automated from `games.csv`, backup automation documented.                             | 1. Design normalized schema (tables for games, platforms, genres, media, prices).<br>2. Add SQL migrations to repo (Supabase CLI).<br>3. Write seeding script to import `games.csv` and enrichment data.<br>4. Add cron or GitHub Action to export database dumps to secure storage.<br>5. Document recovery playbook and data ownership.                                                    |
...
| Track                   | Goal                                | Exit Criteria                                                                                  | Bite-Sized Tasks                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------- | ----------------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dynamic Exploration     | Fast browsing and discovery.        | Virtualized list implemented, instant search with fuzzy matching, scalable navigation shipped. | 1. Integrate virtualized list (e.g., `react-virtualized` equivalent in chosen framework).<br>2. Add multi-field filters (platform, genre, rating, release range).<br>3. Implement typeahead search hitting Supabase indexes.<br>4. Add pagination or infinite scroll fallback for SEO-critical routes.<br>5. Instrument performance metrics (TTFB, search latency).                                                 |
| Collector Tools         | Empower user collection management. | Users can tag statuses, view dashboards, import/export data, and see valuation insights.       | 1. Implement owned/wishlist/backlog/trade states persisted per user.<br>2. Build dashboard components (stats, charts, timelines).<br>3. Add CSV import/export with validation.<br>4. Integrate external APIs (PriceCharting) for pricing data.<br>5. Display price history charts per game and aggregated totals per user.<br>6. Set up background jobs to refresh pricing data.                                    |
| Content Quality & Media | Rich, accessible content.           | Media gallery, metadata completeness targets defined, region/version details surfaced.         | 1. Extend schema for media assets (Supabase Storage/CDN setup).<br>2. Build gallery component with alt text requirements.<br>3. Expand metadata fields (ratings, modes, regions) with UI sections.<br>4. Support region toggles / filters and variant linking.<br>5. Establish contribution workflow for community edits with review queue.<br>6. Add archival strategy (backups for media, checksum verification). |
