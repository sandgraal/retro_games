...
| Collector Tools         | Empower user collection management. | Users can tag statuses, view dashboards, import/export data, and see valuation insights.       | - [x] Implement owned/wishlist/backlog/trade states persisted per user (local-first + share/import).<br>- [ ] Build dashboard components (stats, charts, timelines).<br>- [x] Add CSV/JSON export/import workflows with validation (owned CSV, JSON backup).<br>- [ ] Integrate external APIs (PriceCharting) for pricing data.<br>- [ ] Display price history charts per game and aggregated totals per user.<br>- [ ] Set up background jobs to refresh pricing data.                                    |
