...
try {
  gogAdapter = await import("./sources/gog.js");
} catch {
  // GOG adapter not available - will use generic fetch
}
...
async function fetchSourceRecords(source) {
  ...
  if (source.type === "gog" && gogAdapter) {
    return gogAdapter.fetchGogSource(source);
  }
